...
class LR(Transform):
    ...
    def transform(self,target,vars):
        return lazy_lr(TensorList(target), lr=self.get_settings('lr', params=vars.params), inplace=True)
...
class StepSize(Transform):
    ...
    def transform(self,target,vars):
        ...
        return lazy_lr(TensorList(target), lr=self.get_settings(key, params=vars.params), inplace=True)
...
class Warmup(Transform):
    ...
    @torch.no_grad
    def transform(self,target,vars):
        start_lr, end_lr = self.get_settings('start_lr', 'end_lr', params=vars.params, cls = NumberList)
