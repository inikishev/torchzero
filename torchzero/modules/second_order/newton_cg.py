...
class NewtonMINRES(Module):
    """Newton's method with a matrix-free minimal residual solver.

    This optimizer implements Newton's method using a matrix-free minimal
    residual (MINRES) solver to approximate the search direction. Instead of
    forming the full Hessian matrix, it only requires Hessian-vector products
    (HVPs). These can be calculated efficiently using automatic
    differentiation or approximated using finite differences.

    .. note::
        In most cases NewtonMINRES should be the first module in the chain because it relies on autograd. Use the :code:`inner` argument if you wish to apply Newton preconditioning to another module's output.
    ...
    Args:
        ...
        reg (float, optional):
            Regularization parameter (damping) added to the Hessian diagonal.
            This helps ensure the system is positive-definite. Defaults to 1e-8.
        npc_terminate (float, optional):
            whether to terminate MINRES when non-positive curvature is detected.
        hvp_method (str, optional):
            Determines how Hessian-vector products are evaluated.
        ...
        warm_start (bool, optional):
            ...
            Defaults to False.
        inner (Chainable | None, optional):
            NewtonMINRES will attempt to apply preconditioning to the output of this module.

    Examples:
        Newton-MINRES with a backtracking line search:
        ...
        .. code-block:: python
            ...
            opt = tz.Modular(
                model.parameters(),
                tz.m.NewtonMINRES(maxiter=10, warm_start=True),
                tz.m.Backtracking()
            )

    References:
        This implementation is based on the algorithm described in **Liu, Yang, and Fred Roosta. "MINRES: From negative curvature detection to monotonicity properties." SIAM Journal on Optimization 32.4 (2022): 2636-2661**.
    ...
    def __init__(
        ...
        defaults = dict(tol=tol, maxiter=maxiter, reg=reg, npc_terminate=npc_terminate, hvp_method=hvp_method, h=h, warm_start=warm_start)
        super().__init__(defaults,)
    ...
    def step(self, var):
        ...
        hvp_method = settings['hvp_method']
        h = settings['h']
        npc_terminate = settings['npc_terminate']
        warm_start = settings['warm_start']
        ...
        x = minres(A_mm=H_mm, b=as_tensorlist(b), x0=x0, npc_terminate=npc_terminate, tol=tol, maxiter=maxiter, reg=reg)
        if warm_start:
            assert x0 is not None
            x0.copy_(x)
